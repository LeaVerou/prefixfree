 ...
 * MIT license
 */
(function(l){function k(b,f){return[].slice.call((f||document).querySelectorAll(b))}if(window.getComputedStyle&&window.addEventListener){var b=window.PrefixFree={prefixCSS:function(d,f){function a(a,c,f,i){a=b[a];i=i||"";f=f||"";c=c||i+g+"$1"+f;a.length&&(a=RegExp(i+"("+a.join("|")+")"+f,"gi"),d=d.replace(a,c))}var g=b.prefix;a("functions",g+"$1(","\\s*\\(","\\b");a("keywords",null,"\\b","\\b");a("properties",g+"$1:","\\s*:","\\b");if(b.properties.length){var c=RegExp("\\b("+b.properties.join("|")+
")(?!:)","gi");a("valueProperties",function(a){return a.replace(c,g+"$1")},":(.+?);")}f&&(a("selectors",b.prefixSelector,"\\b"),a("atrules",null,"\\b","@"));return d=d.replace(RegExp("-"+g,"g"),"-")},process:{link:function(d){try{if(!/\bstylesheet\b/i.test(d.rel)||!d.sheet.cssRules)return}catch(f){return}var a=d.getAttribute("href")||d.getAttribute("data-href"),g=a.replace(/[^\/]+$/,""),c=d.parentNode,e=new XMLHttpRequest;e.open("GET",a);e.onreadystatechange=function(){if(e.readyState===4){var a=
e.responseText;if(a&&d.parentNode){var a=b.prefixCSS(a,true),a=a.replace(/url\((?:'|")?(.+?)(?:'|")?\)/gi,function(a,b){return!/^([a-z]{3,10}:|\/)/i.test(b)?'url("'+g+b+'")':a}),f=document.createElement("style");f.textContent=a;c.insertBefore(f,d);c.removeChild(d)}}};e.send(null)},styleElement:function(d){d.textContent=b.prefixCSS(d.textContent,true)},styleAttribute:function(d){var f=d.getAttribute("style"),f=b.prefixCSS(f);d.setAttribute("style",f)}},prefixSelector:function(d){return d.replace(/^:{1,2}/,
