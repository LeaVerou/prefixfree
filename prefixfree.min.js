...
this.fixers||[];this.fixers.push(a)},fix:function(a,b){for(var c=0;c<this.fixers.length;c++)a=this.fixers[c](a,b)||a;return a},camelCase:function(a){return a.replace(/-([a-z])/g,function(a,c){return c.toUpperCase()}).replace("-","")},deCamelCase:function(a){return a.replace(/[A-Z]/g,function(a){return"-"+a.toLowerCase()})}};setTimeout(function(){function a(a,c){return[].slice.call((c||document).querySelectorAll(a))}a('link[rel~="stylesheet"]').forEach(StyleFix.link);document.addEventListener("DOMContentLoaded",
function(){a('link[rel~="stylesheet"]:not([data-inprogress])').forEach(StyleFix.link);a("style").forEach(StyleFix.styleElement);a("[style]").forEach(StyleFix.styleAttribute)},false)},10)}})();
